...
def get_lbs_for_all_corners(crop_size, data_shape):
    """
    Determines the lower bounds for cropping at all corners of the data.

    :param crop_size: The size of the crop (tuple or list of integers).
    :param data_shape: The full dimensions of the data including batch and channel dimensions.
    :return: A list containing lists of lower bound indices for each corner in each spatial dimension.
    """
    # Initialize the list to hold all corners' lower bounds
    all_corners_lbs = []
    # Get the number of spatial dimensions (excluding batch and channel dimensions)
    ...
    for i in range(dim):
        lower_bound = 0
        upper_bound = data_shape[i + 2] - crop_size[i]
        corner_indices.append([lower_bound, upper_bound])

    # Use itertools.product to find all combinations of lower bounds for each dimension
    for corner in itertools.product(*corner_indices):
        all_corners_lbs.append(list(corner))

    return all_corners_lbs
...
def crop(data, seg=None, crop_size=128, margins=(0, 0, 0), crop_type="center",
    ...
    for b in range(data_shape[0]):
        ...
        if crop_type == "center":
            ...
            lbs = get_lbs_for_all_corners(crop_size, data_shape_here)
        else:
            raise NotImplementedError("crop_type must be 'center', 'random', or 'corner'")
