...
from .unet3d import PlainUNet3D, PreActUNet3D, ResidualUNet3D
...
def get_model(config):
    ...
    if model_name == 'plane_unet_3d':
        model = PlainUNet3D(in_channels=config.MODEL.INPUT_CHANNELS,
                            ...
                            residue=False)
    elif model_name == 'pre_act_unet_3d':
        model = PreActUNet3D(in_channels=config.MODEL.INPUT_CHANNELS,
                             out_channels=config.MODEL.OUTPUT_CHANNELS,
                             base_feature_channels=config.MODEL.BASE_FEATURE_CHANNELS,
                             max_feature_channels=config.MODEL.MAX_FEATURE_CHANNELS,
                             base_module=config.MODEL.BASE_MODULE,
                             ...
                             pre_activation=True,
                             residue=True)
    elif model_name == 'residual_unet_3d':
        model = ResidualUNet3D(in_channels=config.MODEL.INPUT_CHANNELS,
                               out_channels=config.MODEL.OUTPUT_CHANNELS,
                               base_feature_channels=config.MODEL.BASE_FEATURE_CHANNELS,
                               max_feature_channels=config.MODEL.MAX_FEATURE_CHANNELS,
                               base_module=config.MODEL.BASE_MODULE,
