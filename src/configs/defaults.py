...
# Data
_C.DATA = CN()
_C.DATA.KITS19_DIR = '../kits19_data/data'
_C.DATA.TRAIN_CASES = list(range(0, 210))
_C.DATA.TEST_CASES = list(range(210, 300))
_C.DATA.CASES_TO_EXCLUDE = []  # see 'An attempt at beating the 3D U-Net' paper
_C.DATA.KITS19_RESAMPLED_DIR = '../kits19_data/data/kits19_resampled'
...
# DataLoader
_C.DATALOADER = CN()
_C.DATALOADER.TRAIN_BATCH_SIZE = 2  # 2 in 'An attempt at beating the 3D U-Net' paper
...
_C.DATALOADER.FOREGROUND_WEIGHT = 0.33
...
_C.TRANSFORM.TRAIN_CROP_TYPE = 'random'  # ['random', 'corner']
...
# random rotation
_C.TRANSFORM.ENABLE_ROTATION = False
...
# Model
_C.MODEL = CN()
_C.MODEL.NAME = 'residual_unet_3d'  # ['plane_unet_3d', 'pre_act_unet_3d', 'residual_unet_3d']
...
def update_output_dir(config):
    ...
    new_dir = f"{base_dir}/{model_name}_fold{fold_id}_epochs{epochs}"
...
def get_default_config():
    """Get default config and update OUTPUT_DIR based on fold_id, num_epoch, and architecture."""
    config = _C.clone()
    config = update_output_dir(config)
    return config
